...
pdf("clusteringEval_RESULTS/abundance_samples.pdf",width=12) 
plot1=ggplot(top20_f1,aes(x=reorder(Specie.strain,Abundance),y=Abundance))+geom_bar(stat="identity")+coord_flip()+labs(title="Sample01",y="Abundance (number of reads)",x="Strain")+theme(plot.title=element_text(size=20),axis.text.x=element_text(size=14),axis.title.x=element_text(size=16),axis.text.y=element_text(size=14),axis.title.y=element_text(size=16),plot.margin=unit(c(0.5,1,0.5,0.5),"cm"))
